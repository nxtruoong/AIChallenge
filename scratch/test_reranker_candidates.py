...
if not engine.reranker:
    print("Error: No reranker loaded in SearchEngine.")
    sys.exit(1)

# Set max length on tokenizer/model
engine.reranker.max_length = 96

print("Pre-computing stage 1 candidates and rerank scores (pool=25, max_len=96)...")
query_candidates = []
...
for item in tqdm(queries_data, desc="Stage 1 + Rerank"):
    ...
    # SigLIP visual
    with torch.no_grad():
        inputs = engine.siglip_processor(text=[q_text], return_tensors="pt", padding=True, truncation=True, max_length=48).to(engine.device)
...
print("\n=== Baseline (Stage-1 RRF Only, No Rerank) ===")
...
print(f"\nBest Config: {best_cfg} -> Recall@1: {best_r1:.2f}%")
